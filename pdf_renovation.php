...
$pdf->Cell( 180 , 7 , iconv( 'UTF-8','cp874' , "หลักฐานการวางเงินประกัน" ),0,1,'C');    
$pdf->SetFont('cordia','B',16);
$pdf->Cell( 180 , 7 , iconv( 'UTF-8','cp874' , "$Company" ),0,1,'C'); 

$pdf->SetXY(16,45);
$pdf->SetFont('cordia','B',16);
$pdf->Cell( 90 , 7 , iconv( 'UTF-8','cp874' , "ได้รับเงินจาก" ),0,0,''); 
...
$pdf->ln(); 
$pdf->ln(); 
$pdf->Cell( 90 , 7 , iconv( 'UTF-8','cp874' , "ลงชื่อ..........................................................ผู้รับเงิน" ),0,0,''); 
$pdf->Cell( 90 , 7 , iconv( 'UTF-8','cp874' , "ลงชื่อ..........................................................ผู้จ่ายเงิน" ),0,1,'');
...
$pdf->Cell( 90 , 7 , iconv( 'UTF-8','cp874' , "                         ผู้วางเงินประกัน" ),0,1,'');
$pdf->ln(); 
$pdf->ln(); 
$pdf->SetFont('cordia','U',16);
$pdf->Cell( 85 , 7 , iconv( 'UTF-8','cp874' , "รายการที่ต้องการต่อเติม มีรายละเอียดดังนี้" ),0,0,'');


//fill DATA
$pdf->SetFont('cordia','',16);
$renovation = mysqli_fetch_array($result_renovation);
$fulldate = thaifulldate2($renovation['RVDate']);
$RVDeposit = number_format($renovation['RVDeposit'],2);
if($renovation['RVDeposittype']==1){
    $note = "(เงินสด)";
}else{
    $note = "(".$renovation['RVDepositcheque'].")";
}
$baht = bahttext($renovation['RVDeposit']);
$user = showUserFullName($link, $renovation['CreateBy']);

$pdf->SetXY(16,30);
$pdf->Cell( 180 , 7 , iconv( 'UTF-8','cp874' , "$fulldate" ),0,0,'R'); 
$pdf->SetXY(40,45);
$pdf->Cell( 90 , 7 , iconv( 'UTF-8','cp874' , "$renovation[RVDepositName]" ),0,0,''); 
$pdf->Cell( 60 , 7 , iconv( 'UTF-8','cp874' , "$renovation[ParAddress]" ),0,1,'');


$pdf->SetXY(16,60);
$pdf->Cell( 15 , 7 , iconv( 'UTF-8','cp874' , "1" ),0,0,'C');
$pdf->Cell( 70 , 7 , iconv( 'UTF-8','cp874' , "วางเงินประกันเพื่อต่อเติมบ้าน" ),0,0,'');
$pdf->Cell( 25 , 7 , iconv( 'UTF-8','cp874' , "$RVDeposit" ),0,0,'R');
...
$pdf->SetFont('cordia','',16);
$pdf->SetXY(25,114);
$pdf->Cell( 60 , 7 , iconv( 'UTF-8','cp874' , "$user" ),0,1,'C'); 
$pdf->SetXY(115,114);
$pdf->Cell( 60 , 7 , iconv( 'UTF-8','cp874' , "$renovation[RVDepositName]" ),0,0,'C'); 
...
while($sub = mysqli_fetch_array($result_sub)){
   $pdf->Cell( 180 , 7 , iconv( 'UTF-8','cp874' , "    $i. $sub[RVSubDetail]" ),0,1,'');   
   $i++;
}
