...
if(isset($_POST["submit"])){
    ...
    // Check if image file is a actual image or fake image
        ...
        mysqli_query($link, "INSERT INTO `attachments` (`RefId`, `ACType`, `BId`, `ACName`, `ACStatus`, `CreateBy`, `CreateDate`) "
                . "VALUES ('$RVId','attachstart','$BId','$ACName','1','$CreateBy', '$CreateDate')");
...
?>
        <div class="modal-header">
          ...
          <h4 class="modal-title"><?php echo "แนบไฟล์ต่อเติม"; ?></h4>
        </div>          
        <div class="modal-body">
            <?php 
            $result_attachments = mysqli_query($link, "SELECT * FROM `attachments` WHERE `RefId` = '$RVId' AND `BId` = '$BId' AND `ACType` = 'attachstart'");
            ...
            <br>
            <hr>
            <form action="modal_renovateattachstart.php" method="post" enctype="multipart/form-data" >
                
                
                กรุณาเลือกไฟล์ที่ต้องการอัพโหลด
                <input type="hidden" name="id" value="<?php echo $id; ?>">
                <input type="hidden" name="RVId" value="<?php echo $RVId; ?>">
                <input type="file" id="image-file" name="fileToUpload" id="fileToUpload" accept=".jpg,.pdf,.jpeg,.png" size="4MB" >
